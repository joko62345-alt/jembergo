...
class MidtransGateway implements PaymentGateway
{
    ...
    {
        return $this->charge($booking, $amount ?? (float) $booking->total_harga, 'JGO-'.$booking->kode_booking);
    }

    public function createPaymentForAmount(Pemesanan $booking, string $method, float $amount): array
    {
        return $this->charge($booking, $amount, 'JGO-'.$booking->kode_booking.'-CHG-'.strtoupper(Str::random(8)));
    }
    ...
    private function charge(Pemesanan $booking, float $amount, string $orderId): array
    {
        ...
        abort_if(! config('services.payment_gateway.midtrans_server_key'), 503, 'Midtrans belum dikonfigurasi. Isi MIDTRANS_SERVER_KEY di .env.');
        ...
        if ($response->failed()) {
            throw new RuntimeException((string) ($response->json('status_message') ?: 'Transaksi QRIS gagal dibuat di Midtrans.'));
        }
    ...
    {
        $baseUrl = config('services.payment_gateway.midtrans_is_production')
            ? 'https://api.midtrans.com'
            : 'https://api.sandbox.midtrans.com';

        return Http::baseUrl($baseUrl)
            ->withBasicAuth((string) config('services.payment_gateway.midtrans_server_key'), '')
            ->acceptJson()
            ->asJson()
            ->timeout(20);
    }
