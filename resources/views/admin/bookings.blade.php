<x-admin-layout title="Pemesanan Destinasi" active="bookings">
    ...
    <div class="card border-0 shadow-sm rounded-4"><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Booking & pemesan</th><th>Peserta</th><th>Kunjungan</th><th>Pembayaran</th><th>Status tiket</th><th>Total</th></tr></thead><tbody>
        @forelse($bookings as $booking)
            <tr><td><strong>{{ $booking->kode_booking }}</strong><small class="d-block">{{ $booking->customer->nama ?? '-' }}</small><small class="d-block text-secondary">{{ $booking->customer->email ?? '-' }}</small></td><td><strong>{{ $booking->ketua_nama }}</strong> <span class="badge text-bg-light">Ketua</span>@foreach($booking->anggota_names ?? [] as $member)<small class="d-block">{{ data_get($member, 'nama', $member) }} <span class="text-secondary">(Anggota)</span></small>@endforeach</td><td>{{ $booking->tanggal_kunjungan->format('d/m/Y') }}<small class="d-block text-secondary">{{ \App\Support\StatusLabel::order($booking->status_pemesanan) }}</small></td><td><span class="badge {{ $booking->pembayaran?->status_pembayaran === 'PAID' ? 'text-bg-success' : 'text-bg-warning' }}">{{ \App\Support\StatusLabel::payment($booking->pembayaran?->status_pembayaran) }}</span><small class="d-block text-secondary mt-1">{{ $booking->pembayaran->metode_pembayaran ?? '-' }}</small></td><td>@forelse($booking->tiket as $ticket)<div class="d-flex align-items-center gap-2 mb-1"><span class="badge {{ $ticket->status_tiket === 'USED' ? 'text-bg-success' : ($ticket->status_tiket === 'ACTIVE' ? 'text-bg-primary' : 'text-bg-secondary') }}">{{ \App\Support\StatusLabel::ticket($ticket->status_tiket) }}</span><small>{{ $ticket->waktu_verifikasi?->setTimezone(config('app.timezone'))->format('d/m H:i') ?? '-' }}</small></div>@empty<span class="text-secondary small">Belum dibuat</span>@endforelse</td><td class="text-nowrap">Rp {{ number_format($booking->total_harga, 0, ',', '.') }}</td></tr>
        @empty
