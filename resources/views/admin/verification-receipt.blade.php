@php
    $participants = collect($booking->anggota_names ?? [])->prepend([
...
<head>
    ...
    <style>
        :root { color: #17212b; font-family: Arial, Helvetica, sans-serif; }
        * { box-sizing: border-box; }
        body { margin: 0; background: #eef2f5; }
        .receipt-actions { display: flex; justify-content: center; gap: .75rem; padding: 1.5rem 1rem; }
        .receipt-actions a, .receipt-actions button { border: 0; border-radius: .45rem; padding: .7rem 1rem; background: #173b60; color: #fff; cursor: pointer; font: inherit; text-decoration: none; }
        .receipt-actions .secondary { background: #fff; color: #173b60; box-shadow: inset 0 0 0 1px #c8d2dc; }
        .receipt { width: 80mm; max-width: calc(100% - 2rem); margin: 0 auto 2rem; padding: 5mm 4mm; background: #fff; box-shadow: 0 8px 30px rgba(23, 59, 96, .12); font-size: 11px; line-height: 1.35; }
        .receipt-header { text-align: center; }
        .receipt-brand { margin: 0; font-size: 21px; letter-spacing: -.5px; }
        .receipt-brand span { color: #f28a00; }
        .receipt-header p { margin: 2px 0; }
        .muted { color: #637181; }
        .rule { margin: 9px 0; border-top: 1px dashed #68737d; }
        ...
        @media print {
            ...
            .receipt { width: 80mm; max-width: none; margin: 0; padding: 4mm; box-shadow: none; }
        }
    </style>
</head>
<body>
    ...
    <main class="receipt">
        ...
        <div class="rule"></div>
        <div class="receipt-row"><span>Kode booking</span><strong>{{ $booking->kode_booking }}</strong></div>
        <div class="receipt-row"><span>Tanggal kunjungan</span><strong>{{ $booking->tanggal_kunjungan->format('d/m/Y') }}</strong></div>
        <div class="receipt-row"><span>Waktu verifikasi</span><strong>{{ $verifiedTickets->first()?->waktu_verifikasi?->setTimezone(config('app.timezone'))->format('d/m/Y H:i') ?? now()->format('d/m/Y H:i') }}</strong></div>
        <div class="rule"></div>
