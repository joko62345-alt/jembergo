...
<head>
    ...
    <style>
        ...
        .auth-premium-shell {
            ...
            height: min(650px, calc(100vh - clamp(1.5rem, 2.5vw, 2.5rem)));
            min-height: 0;
            overflow: hidden;
            border: 1px solid rgba(255, 255, 255, .8);
            border-radius: 2rem;
            background: #fff;
            box-shadow: 0 24px 70px rgba(23, 59, 96, .16);
        }
        ...
        .auth-form-panel {
            ...
            overflow-y: auto;
            overflow-x: hidden;
            padding: 1.75rem clamp(1.5rem, 4vw, 4.5rem);
            background: #fffefa;
        }
        ...
        .auth-photo-panel {
            ...
            z-index: 1;
            overflow: hidden;
            background: #123b78 url('{{ route('assets.login') }}') center / cover no-repeat;
        }

        .auth-photo-panel::before {
            position: absolute;
            inset: 0;
            background: linear-gradient(145deg, rgba(9, 31, 57, .08), rgba(9, 31, 57, .55));
            content: '';
        }

        .auth-premium-shell.is-register .auth-form-panel {
            transform: translateX(100%);
        }

        .auth-premium-shell.is-register .auth-photo-panel {
            transform: translateX(-100%);
        }
        ...
        .auth-field .form-control:focus {
            border-color: #7fbbdb;
            background: #fff;
            box-shadow: 0 0 0 3px rgba(127, 187, 221, .16);
        }

        .auth-field .input-group .btn {
            border-color: #dbe6ee;
            background: #f9fcfd;
            color: #7890a1;
        }
        ...
        .auth-submit {
            ...
            font-size: .86rem;
            font-weight: 600;
            box-shadow: 0 8px 18px rgba(231, 125, 0, .2);
        }
        ...
        .auth-photo-content .auth-location {
            display: inline-flex;
            align-items: center;
            gap: .4rem;
            padding: .45rem .7rem;
...
<body>
    <main class="auth-premium-page">
        <section class="auth-premium-shell {{ $authMode === 'register' ? 'is-register' : '' }}"
            id="authShell">
            <div class="auth-panel auth-form-panel">
                <div class="auth-form-inner">
                    ...
                    <p class="auth-subtitle" data-login-subtitle>Masuk untuk menikmati informasi dan
                        layanan pariwisata Kabupaten Jember</p>
                    @if (session('error'))
                        <div class="alert auth-error">{{ session('error') }}</div>
                    @endif
                    @if (session('success'))
                        <div class="alert auth-success" role="status"><i
                                class="bi bi-check-circle"></i><span>{{ session('success') }} <a
                                    href="{{ route('login') }}">Halaman login</a></span><button
                                type="button" class="auth-alert-close" data-dismiss-auth-alert
                                aria-label="Tutup pesan"><i class="bi bi-x-lg"></i></button></div>
                    @endif
                    @if ($errors->any())
                    ...
                    <div data-login-form>
                        <form action="{{ route('login.store') }}" method="POST">
                            ...
                            <div class="auth-field"><label for="login-password">Password</label>
                                <div class="input-group"><input id="login-password" type="password"
                                        name="password" class="form-control"
                                        placeholder="Masukkan password" required><button
                                        type="button" class="btn password-toggle"
                                        data-target="login-password"
                                        aria-label="Tampilkan password"><i
                                            class="bi bi-eye"></i></button></div>
                            </div>
                            <button class="auth-submit" type="submit"><span>Masuk</span><i
                                    class="bi bi-arrow-right"></i></button>
                        </form>
                        <div class="auth-divider"><span>atau</span></div>
                        <a href="{{ route('google.redirect') }}" class="auth-google"><svg
                                class="google-mark" viewBox="0 0 24 24" aria-hidden="true">
                                <path fill="#4285F4"
                                    d="M21.35 12.27c0-.78-.07-1.53-.2-2.27H12v4.3h5.24a4.48 4.48 0 0 1-1.94 2.94v2.45h3.14c1.84-1.7 2.91-4.2 2.91-7.42Z" />
                                <path fill="#34A853"
                                    d="M12 21.8c2.63 0 4.84-.87 6.45-2.36l-3.14-2.45c-.87.58-1.98.93-3.31.93-2.54 0-4.7-1.72-5.47-4.03H3.28v2.53A9.74 9.74 0 0 0 12 21.8Z" />
                                <path fill="#FBBC05"
                                    d="M6.53 13.89a5.86 5.86 0 0 1 0-3.78V7.58H3.28a9.8 9.8 0 0 0 0 8.84l3.25-2.53Z" />
                                <path fill="#EA4335"
                                    d="M12 6.08c1.43 0 2.72.49 3.73 1.45l2.8-2.8C16.84 3.17 14.63 2.2 12 2.2a9.74 9.74 0 0 0-8.72 5.38l3.25 2.53C7.3 7.8 9.46 6.08 12 6.08Z" />
                            </svg><span>Lanjutkan dengan Google</span></a>
                        <div class="auth-extra"><a href="{{ route('password.request') }}">Lupa
                                password?</a></div>
                    </div>
                    <div data-register-form>
                        <h1></h1>
                        <p class="auth-subtitle">Daftar untuk mendapatkan akses ke layanan
                            pariwisata Kabupaten Jember</p>
                        <form action="{{ route('register.store') }}" method="POST">@csrf
                            ...
                            <div class="auth-field"><label
                                    ...
                                    required></div>
                            <button class="auth-submit" type="submit"><span>Daftar</span><i
                                    class="bi bi-arrow-right"></i></button>
                        </form>
                    </div>
                    <p class="auth-switch-row" data-login-switch>Belum memiliki akun? <button
                            class="auth-switch" type="button"
                            data-switch="register">Daftar</button></p>
                    <p class="auth-switch-row" data-register-switch>Sudah memiliki akun? <button
                            class="auth-switch" type="button" data-switch="login">Masuk</button>
                    </p>
                </div>
            </div>
            <div class="auth-panel auth-photo-panel"
                aria-label="Pemandangan wisata Kabupaten Jember">
                <span class="auth-photo-badge"><i class="bi bi-stars"></i> Wonderful Jember</span>
                <div class="auth-photo-content"><span class="auth-location"><i
                            class="bi bi-geo-alt-fill"></i> Kabupaten Jember, Jawa Timur</span>
                    <h2>Jelajahi pesona Jember.</h2>
                    <p>Pantai, perbukitan hijau, dan pengalaman wisata yang menunggu untuk
                        ditemukan.</p>
                </div>
            </div>
        </section>
    ...
    <script>
        ...
        const setMode = mode => {
            ...
            history.replaceState(null, '', register ? '{{ route('register') }}' :
                '{{ route('login') }}');
        };
        ...
        document.querySelector('[data-dismiss-auth-alert]')?.addEventListener('click', event => event
            .currentTarget.closest('.auth-success').remove());
        ...
        const validateRegisterName = () => {
            const invalid = registerNameInput.value.length > 0 && !/^[\p{L}\s]+$/u.test(
