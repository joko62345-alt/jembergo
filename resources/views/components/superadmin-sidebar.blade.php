<aside id="superadmin-sidebar" class="superadmin-sidebar">
    ...
    <nav class="superadmin-nav" aria-label="Navigasi Super Admin">
        ...
        @if(session('jg_role') === 'SUPER_ADMIN')
            <a href="{{ route('superadmin.destinations') }}" class="nav-link {{ request()->routeIs('superadmin.destinations*') ? 'active' : '' }}" title="Destinasi wisata"><i class="bi bi-geo-alt" aria-hidden="true"></i><span>Destinasi wisata</span></a>
            @php($managementOpen = request()->routeIs('superadmin.management*'))
            <div class="sidebar-dropdown {{ $managementOpen ? 'is-open' : '' }}">
                <button type="button" class="nav-link sidebar-dropdown-toggle {{ $managementOpen ? 'active' : '' }}" aria-expanded="{{ $managementOpen ? 'true' : 'false' }}" aria-controls="management-submenu" title="Manajemen akun">
                ...
                <div class="sidebar-submenu" id="management-submenu">
                    <a href="{{ route('superadmin.management.admins') }}" class="nav-link {{ request()->routeIs('superadmin.management.admins') ? 'active' : '' }}" title="Admin Pariwisata"><i class="bi bi-person" aria-hidden="true"></i><span>Admin Pariwisata</span></a>
                    <a href="{{ route('superadmin.management.customers') }}" class="nav-link {{ request()->routeIs('superadmin.management.customers') ? 'active' : '' }}" title="Customer"><i class="bi bi-person" aria-hidden="true"></i><span>Customer</span></a>
                </div>
            </div>
            <a href="{{ route('superadmin.articles') }}" class="nav-link {{ request()->routeIs('superadmin.articles*') ? 'active' : '' }}" title="Artikel"><i class="bi bi-journal-text" aria-hidden="true"></i><span>Artikel</span></a>
            <a href="{{ route('superadmin.report') }}" class="nav-link {{ request()->routeIs('superadmin.report*') ? 'active' : '' }}" title="Laporan"><i class="bi bi-bar-chart" aria-hidden="true"></i><span>Laporan</span></a>
        ...
        @else
            ...
            <a href="{{ route('customer.profile') }}" class="nav-link" title="Profil"><i class="bi bi-person" aria-hidden="true"></i><span>Profil</span></a>
        @endif
    </nav>
    <form method="POST" action="{{ route('logout') }}" class="superadmin-logout">@csrf<button class="nav-link" type="submit" title="Keluar"><i class="bi bi-box-arrow-left" aria-hidden="true"></i><span>Keluar</span></button></form>
</aside>
<script>
    ...
    if (sidebarCollapseToggle && sidebar && localStorage.getItem('superadmin-sidebar-collapsed') === 'true') {
    ...
    sidebarCollapseToggle?.addEventListener('click', () => {
        ...
        sidebarCollapseToggle.setAttribute('aria-label', collapsed ? 'Lebarkan menu navigasi' : 'Ciutkan menu navigasi');
        sidebarCollapseToggle.setAttribute('title', collapsed ? 'Lebarkan menu navigasi' : 'Ciutkan menu navigasi');
        sidebarCollapseToggle.innerHTML = `<i class="bi bi-layout-sidebar${collapsed ? '' : '-inset'}" aria-hidden="true"></i>`;
    });
