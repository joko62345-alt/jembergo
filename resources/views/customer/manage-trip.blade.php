            ...
            <div class="manage-layout">
                <section class="manage-main">
                    <article class="manage-card">
                        ...
                        <input id="tanggal_kunjungan" name="tanggal_kunjungan" type="date" min="{{ today()->toDateString() }}" max="{{ now()->addMonths(2)->format('Y-m-d') }}" value="{{ old('tanggal_kunjungan', $booking->tanggal_kunjungan->toDateString()) }}" required>
...
<style>
    ...
    @media (max-width: 800px) { .manage-header { align-items:start; flex-direction:column; }.manage-layout { grid-template-columns:1fr; }.manage-main { max-height:none; overflow:visible; padding-right:0; }.manage-summary, .summary-card { position:static; }.manage-steps { overflow:auto; }.manage-steps i { flex:0 0 auto; }.manage-steps div { flex:0 0 auto; } }
...
(() => {
    ...
    list.addEventListener('input', () => { refresh(); updateReviewState(); }); list.addEventListener('change', () => { refresh(); updateReviewState(); }); list.addEventListener('click', event => { const button = event.target.closest('.remove-member'); if (button) { button.closest('[data-member-row]').remove(); add.disabled = false; refresh(); updateReviewState(); } }); visitDate?.addEventListener('change', updateReviewState); visitDate?.addEventListener('input', updateReviewState); refresh(); updateReviewState();
})();
</script>
@endsection
