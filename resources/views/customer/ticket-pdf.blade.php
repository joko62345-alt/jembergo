...
<body>
    ...
    <header class="header">
        <div class="header-meta"><strong>BUKTI BOOKING ONLINE</strong><br>Dokumen e-ticket
            kelompok<br>{{ now()->format('d/m/Y H:i') }}</div>
    </header>

    <div class="document-title">BUKTI PEMESANAN TIKET WISATA<span>Harap tunjukkan QR grup kepada
            petugas destinasi</span></div>
    ...
    <table class="details">
        <tr>
            ...
            <td>Destinasi</td>
            <td>{{ $booking->destinasi->nama_wisata }}</td>
        </tr>
        <tr>
            <td>Tanggal pesan</td>
            <td>{{ $booking->tanggal_pemesanan->format('d/m/Y H:i') }}</td>
            <td>Tanggal kunjungan</td>
            <td>{{ $booking->tanggal_kunjungan->format('d/m/Y') }}</td>
        </tr>
        <tr>
    ...
    <table class="details">
        <tr>
            <td>Nama</td>
            <td>{{ $booking->ketua_nama }}</td>
            <td>Email</td>
            <td>{{ $booking->ketua_email }}</td>
        </tr>
        <tr>
            <td>Nomor HP</td>
            <td>{{ $booking->ketua_no_hp }}</td>
            <td>Status</td>
            <td>{{ $groupTicket ? \App\Support\StatusLabel::ticket($groupTicket->status_tiket) : 'Belum diterbitkan' }}
            </td>
        </tr>
    </table>
        ...
        <tbody>
            @foreach ($participants as $participant)
                <tr>
                    ...
                    <td>{{ $booking->detailPemesanan->firstWhere('id_jenis_tiket', (int) $participant['id_jenis_tiket'])?->jenisTiket?->nama_jenis ?? 'Tiket wisata' }}
                    </td>
                    <td class="status">
                        {{ $groupTicket ? \App\Support\StatusLabel::ticket($groupTicket->status_tiket) : 'Belum diterbitkan' }}
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <div class="notes"><strong>Catatan:</strong><br>• QR code berlaku untuk seluruh peserta dalam
        satu booking.<br>• Tunjukkan dokumen ini kepada petugas saat tiba di destinasi.<br>•
        E-ticket berlaku sesuai tanggal kunjungan dan status pembayaran.<br>• Data pada dokumen ini
        diterbitkan oleh JemberGo.</div>
</body>
