...
<section class="mt-5"><h2 class="h4 fw-bold mb-3">Ulasan pengunjung</h2><div class="destination-review-scroller">@forelse($destination->review->sortByDesc('tanggal_review')->take(5) as $review)<article class="destination-review-item"><div class="d-flex justify-content-between align-items-start gap-2"><strong>{{ $review->customer->nama }}</strong><span class="review-stars">{{ str_repeat('★', $review->rating) }}</span></div><p>{{ $review->ulasan }}</p></article>@empty<p class="text-secondary">Belum ada ulasan.</p>@endforelse</div></section></main>
