...
@section('content')
    <div class="page-heading">
        <span class="page-kicker">Manajemen pengguna</span>
        <h1>Buat akun Admin Pariwisata</h1>
        <p>Akun ini dibuat oleh Super Admin dan digunakan untuk mengelola satu destinasi.</p>
    </div>
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body p-4 p-lg-5">
                    ...
                    @endif
                    <form method="POST" action="{{ route('superadmin.management.admin') }}"
                        class="row g-3">@csrf
                        ...
                        <div class="col-md-6"><label class="form-label"
                                ...
                                type="email" class="form-control" placeholder="nama@gmail.com"
                                value="{{ old('email') }}" required><small id="email_warning"
                                class="text-danger" hidden>Email admin harus menggunakan
                                @gmail.com.</small></div>
                        <div class="col-md-6"><label class="form-label" for="no_hp">Nomor
                                ...
                                hidden>Nomor HP tersebut sudah digunakan oleh admin pariwisata
                                lain.</small></div>
                        <div class="col-md-6"><label class="form-label" for="password">Password</label>
                            <div class="input-group"><input id="password" name="password"
                                    type="password" minlength="8" class="form-control password-input"
                                    placeholder="Masukkan Password" required><button type="button"
                                    class="btn password-toggle" data-target="password"
                                    aria-label="Tampilkan password"><i class="bi bi-eye"
                                        aria-hidden="true"></i></button></div>
                        </div>
                        <div class="col-12"><label class="form-label" for="id_destinasi">Destinasi
                                tugas</label><select id="id_destinasi" name="id_destinasi"
                                class="form-select @error('id_destinasi') is-invalid @enderror"
                                required>
                                <option value="">Pilih destinasi tugas</option>
                                @foreach ($destinations as $destination)
                                    <option value="{{ $destination->id_destinasi }}"
                                        data-destination-name="{{ $destination->nama_wisata }}"
                                        @selected(old('id_destinasi') == $destination->id_destinasi)>{{ $destination->nama_wisata }}
                                    </option>
                                @endforeach
                            </select><small id="destination_warning" class="text-danger" role="alert"
                                @if (!$errors->has('id_destinasi')) hidden @endif>
                                @error('id_destinasi')
                                    {{ $message }}
                                @else
    ...
    <script>
        ...
        const duplicateFields = [{
            ...
            },
            {
                ...
                key: 'email',
                message: 'Email tersebut sudah digunakan oleh admin lain.'
            },
        ];
        ...
        const validateAdminEmail = () => {
            const invalidDomain = Boolean(emailInput?.value.trim()) && !/@gmail\.com$/i.test(
                emailInput.value.trim());
            const duplicate = existingAdmins.some((admin) => normalizeDuplicateValue(admin
                .email) === normalizeDuplicateValue(emailInput?.value));
            const message = invalidDomain ? 'Email admin harus menggunakan @gmail.com.' :
            ...
            if (emailWarning) {
                ...
                emailWarning.hidden = !invalidDomain && !duplicate;
            }
        };
        ...
        const normalizeDestinationName = (value) => String(value || '').trim().replace(/^admin\s+/i, '')
        ...
        const validateAdminDestination = () => {
            ...
            if (destinationWarning) {
                destinationWarning.textContent = message;
                destinationWarning.hidden = !hasMismatch;
            }
            return !hasMismatch;
        };
        const validateDuplicates = () => duplicateFields.forEach(({
            input,
            warning,
            key,
            message
        }) => {
            const duplicate = Boolean(input.value.trim()) && existingAdmins.some((admin) =>
                normalizeDuplicateValue(admin[key]) === normalizeDuplicateValue(input.value)
            );
        ...
        document.querySelectorAll('.password-toggle').forEach((button) => button.addEventListener(
            'click', () => {
                ...
                button.innerHTML =
                    `<i class="bi bi-eye${visible ? '' : '-slash'}" aria-hidden="true"></i>`;
            }));
    </script>
@endpush
