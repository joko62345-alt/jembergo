...
@section('content')
    ...
    <div class="row"><div class="col-lg-8"><div class="card"><div class="card-body p-4 p-lg-5">
        ...
        <form method="POST" action="{{ route('superadmin.management.admin') }}" class="row g-3">@csrf
            <div class="col-md-6"><label class="form-label" for="nama">Nama</label><input id="nama" name="nama" class="form-control" placeholder="Contoh: Budi Santoso" value="{{ old('nama') }}" required></div>
            <div class="col-md-6"><label class="form-label" for="email">Email</label><input id="email" name="email" type="email" class="form-control" placeholder="nama@email.com" value="{{ old('email') }}" required></div>
            <div class="col-md-6"><label class="form-label" for="no_hp">Nomor HP</label><input id="no_hp" name="no_hp" type="tel" inputmode="numeric" pattern="[0-9]+" class="form-control" placeholder="081234567890" value="{{ old('no_hp') }}" required><small id="no_hp_warning" class="text-danger" hidden>Nomor HP hanya boleh berisi angka.</small></div>
            <div class="col-md-6"><label class="form-label" for="password">Password</label><div class="input-group"><input id="password" name="password" type="password" minlength="8" class="form-control password-input" placeholder="Minimal 8 karakter" required><button type="button" class="btn password-toggle" data-target="password" aria-label="Tampilkan password"><i class="bi bi-eye" aria-hidden="true"></i></button></div></div>
